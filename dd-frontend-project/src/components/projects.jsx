      ...
      <div className="bg-customGreen">
        <section className="pt-[100px] pb-[100px] px-6 md:px-8 lg:px-60">
          ...
          {data.list.map((project) => (
            <div
              key={project.id}
              className="bg-white rounded-[18px] flex flex-col md:flex-row items-stretch "
            >
              ...
              <div className="flex-1 p-6">
                ...
                <p className="text-[#383838] mt-4">{project.description}</p>
                <div className="mt-4 space-x-2">
                  {project.technologies.map((tech, index) => (
                    <button
                      key={index}
                      className="bg-customPurple text-white rounded px-4"
                    >
                      {tech}
                    </button>
                  ))}
